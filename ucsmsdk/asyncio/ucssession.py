...
class UcsSession(object):
    ...
    async def _unfreeze(self, params_json):
        ...
        if self.__auto_refresh and not await self.__validate_connection():
            await self._refresh(auto_relogin=True)
    ...
    async def post(self, uri, data=None, read=True, timeout=None):
        """
        ...
        response = await self.__driver.post(
            uri=uri,
            data=data,
            read=read,
            timeout=timeout
    ...
    async def post_xml(self, xml_str, read=True, timeout=None):
        """
        ...
        response_str = await self.post(uri=ucsm_uri, data=xml_str, read=read, timeout=timeout)
    ...
    async def post_elem(self, elem, timeout=None):
        """
        ...
        await self._tx_lock_acquire_conditional(elem)
        ...
        try:
            response_str = await self.post_xml(xml_str, timeout=timeout)
    ...
    async def _tx_lock_acquire_conditional(self, elem):
        """
    ...
    async def _refresh(self, auto_relogin=False):
        """
        ...
        # re-enable the timer
        await self.__start_refresh_timer()
        return True

    async def __is_ucsm(self, timeout=None):
        """
        ...
        try:
            nw_elem_response = await self.post_elem(nw_elem, timeout=timeout)
    ...
    async def __validate_connection(self, timeout=None):
        """
    ...
    async def _update_version(self, response=None):
        from ..ucscoremeta import UcsVersion
        from ..ucsmethodfactory import config_resolve_dn
        from ..mometa.top.TopSystem import TopSystem
    ...
    async def _update_domain_name_and_ip(self):
        from ..ucsmethodfactory import config_resolve_dn
        from ..mometa.top.TopSystem import TopSystem
    ...
    async def _login(self, auto_refresh=False, force=False, timeout=None):
        """
        ...
        if await self.__validate_connection(timeout=timeout):
            return True
        ...
        if not await self.__is_ucsm(timeout=timeout):
            raise UcsLoginError("Not a supported server.")

        await self._update_version(response)
        await self._update_domain_name_and_ip()
    ...
    async def _logout(self, timeout=None):
        """
